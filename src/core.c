...
DEF_PRIMITIVE(w_Object_equal)
{
	RETURN_BOOL(AS_OBJ(args[0]) == AS_OBJ(args[1]));
}
...
DEF_PRIMITIVE(w_Set_subscript)
{
	ObjSet* set = AS_SET(args[0]);

	if (IS_NUMBER(args[1]))
	{
		uint32_t index = validate_index(args[1], set->elements.count, "Subscript");
		if (index == UINT32_MAX) {
			RETURN_FALSE;
		}
